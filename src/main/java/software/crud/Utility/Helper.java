...
public class Helper {
    ...
    private static final Pattern FIRST_CHAR_FOLLOWED_BY_UPPERCASE = Pattern.compile("(?<=[A-Z])[A-Z0-9]+$");
    private static final Pattern LOWERCASE_NEXT_TO_NUMBER = Pattern.compile("(?<=[0-9])[a-z]");
    private static final Pattern UPPERCASE_INSIDE = Pattern.compile("(?<=[A-Z])[A-Z]+?((?=[A-Z][a-z])|(?=[0-9]))");
    ...
    public static String toPascalCase(String original) {
        return UNDERSCORES.splitAsStream(SPECIAL_CHARACTERS.matcher(original).replaceAll("_"))
                .filter(w -> !w.isEmpty())
                .map(w -> STARTS_WITH_LOWERCASE.matcher(w).replaceFirst(m -> m.group().toUpperCase()))
                .map(w -> FIRST_CHAR_FOLLOWED_BY_UPPERCASE.matcher(w).replaceFirst(m -> m.group().toLowerCase()))
                .map(w -> LOWERCASE_NEXT_TO_NUMBER.matcher(w).replaceFirst(m -> m.group().toUpperCase()))
                .map(w -> UPPERCASE_INSIDE.matcher(w).replaceFirst(m -> m.group().toLowerCase()))
